...
/**
 * Fetch projects from Alnair API
 * @param {Object} params - Query parameters for Alnair API
 * @returns {Promise<Array>} - Array of projects from Alnair
 */
export async function fetchProjectsFromAlnair(params = {}) {
  try {
    // Import the Alnair service dynamically to avoid circular dependencies
    const { findProjects } = await import('./alnairService');
    
    // Default to Dubai if no parameters specified
    const searchParams = {
      ...params,
      limit: params.limit || 100,
    };
    
    const result = await findProjects(searchParams);
    
    // Alnair API response shape: { data: { items: [...] }, count, page, pages }
    const items = result?.data?.items;
    if (!items || !Array.isArray(items)) {
      console.warn('No projects returned from Alnair API', result);
      return [];
    }
 ...
 * Searches through all pages of results to find the project, then fetches
 * the full description from /project/look/{slug}.
 * @param {string|number} projectId - Project ID from URL
 * @returns {Promise<Object|null>} - Project with description or null if not found
 */
export async function fetchProjectDetailsFromAlnair(projectId) {
  try {
    ...
    // Search through pages until we find the project
    // Alnair API returns max 100 per page
    for (let page = 1; page <= 5; page++) {
      ...
      // If this page returned fewer than 100, no more pages
      if (items.length < 100) break;
    }

    if (!found) {
      console.warn(`Project ${projectId} not found in Alnair listing`);
      return null;
    }

    // Fetch the full description from /project/look/{slug}
    // This endpoint returns richtext description + more details
    if (found.slug) {
      try {
        const { findProjects: _, ...rest } = await import('./alnairService');
        // Call proxy directly to get description
        ...
        if (detailResponse.ok) {
          ...
          // Merge description and any extra fields from the detail endpoint
          if (detail && !detail.error && detail.title) {
            found = {
              ...found,
              description: detail.description || found.description || null,
              detail_data: detail,
            };
          }
        }
      } catch (descErr) {
        // Description fetch is non-critical — continue without it
        console.warn('Could not fetch project description:', descErr.message);
      }
    }

    return found;
  } catch (error) {
    console.error('Error fetching project details from Alnair:', error);
    throw error;
  }
}
