...
export async function fetchAllDevelopers(forceRefresh = false) {
  ...
  if (
    ...
    developersCache.timestamp &&
    now - developersCache.timestamp < CACHE_DURATION
  ) {
  ...
  developersCache.pending = (async () => {
    try {
      ...
      const firstResponse = await fetch(`/api/developers?page=1&min_projects=1`, {
        method: "GET",
      ...
      if (!firstResponse.ok) {
        throw new Error(
          `API request failed with status ${firstResponse.status}`,
        );
      }
      ...
      if (totalPages > 1) {
        ...
        for (let page = 2; page <= Math.min(totalPages, 10); page++) {
          pagePromises.push(
            fetch(`/api/developers?page=${page}&min_projects=1`, {
              method: "GET",
              headers: { "Content-Type": "application/json" },
            }).then((res) => res.json()),
          );
      ...
      const processedDevelopers = allDevelopers
        ...
        .map((dev) => {
          ...
          if (logo && logo.includes("oss.pixxicrm.com")) {
            logo = logo.replace(
              "https://oss.pixxicrm.com/",
              "https://pixxicrm.ae/api/",
            );
          }
          return {
            id: dev.id,
