...
/**
 * Strip HTML tags and return plain text. Decodes common entities and normalizes whitespace.
 * @param {string} html - HTML string (e.g. from descriptions.json)
...
/**
 * Clean text by stripping HTML (if present) and fixing common encoding issues
 ...
 * @param {Object} params
 * @param {boolean} params.onlyWithDescription - If true, return only projects that have an entry in descriptions.json
 */
export async function fetchProjectsFromAlnair(params = {}) {
  let all = await _loadAllProjects();

  // Optionally restrict to projects that have a description (e.g. for Newly Launched section)
  if (params.onlyWithDescription) {
    const descriptions = await _loadDescriptions();
    all = all.filter((p) => {
      const slug = p.slug;
